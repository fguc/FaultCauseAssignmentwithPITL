...
figure
set(gcf, 'units','normalized','outerposition',[0.0 0.1 0.4 0.7]);
subplot(a,b,numPlot)
plot(dataRaw.r{1},'g','LineWidth',2)
grid on; hold on
plot(dataRaw.ys{1},'b','LineWidth',1)
plot(dataRaw.y{1},'m','LineWidth',1)
title([simData.simDef ' - Wndw: ' num2str(simData.DMDWndw)])

eigStr = num2str(diag(resultsDMDc.eigs));
for idx=1:1:7
    subplot(a,b,idx)
    plot(dataRaw.time(1:length(real(resultsDMDc.Phi(:,idx))),1),real(resultsDMDc.Phi(:,idx)))
